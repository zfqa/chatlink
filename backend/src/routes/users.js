...
router.get('/search', authMiddleware, (req, res) => {
  ...
  let users;
  if (q.trim() === '') {
    users = db.prepare('SELECT id, email, nickname, avatar_url as avatarUrl, signature FROM users WHERE id != ? LIMIT 50').all(req.userId);
  } else {
    users = db.prepare('SELECT id, email, nickname, avatar_url as avatarUrl, signature FROM users WHERE id != ? AND (nickname LIKE ? OR id LIKE ?) LIMIT 50').all(req.userId, '%' + q + '%', '%' + q + '%');
  }
...
router.get('/:userId', authMiddleware, (req, res) => {
  const user = db.prepare('SELECT id, email, nickname, avatar_url as avatarUrl, signature FROM users WHERE id = ?').get(req.params.userId);
  if (!user) return res.status(404).json({ code: 1004, message: 'User not found', data: null });
  res.json({ code: 0, message: 'success', data: user });
});
